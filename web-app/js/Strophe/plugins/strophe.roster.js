...
Strophe.addConnectionPlugin('roster',
{
    ...
    addRosterHandlers:function(conn){
        var roster = this;
        try
           {
               console.log('add roster handler');
               // Presence subscription
    ...
    _onReceiveRosterSuccess: function(userCallback, stanza)
    {
        ...
        if (this._firstRoster){
            this._firstRoster = false;
            if (this._storedPresences.length > 0){
                for(var prez in this._storedPresences){
                    this._onReceivePresence(this._storedPresences[prez]);
                }
            }
            _storedPresences = [];
        }
    ...
    _onReceivePresence : function(presence)
    {
        ...
        {
            if (this._firstRoster){
                console.log("store presence for futur roster: "+from);
                this._storedPresences.push(presence);
            }else{
                console.log("receive presence not in roster: "+from);
            }
            return true;
        }
        if (type == 'unavailable')
        ...
        {
            // TODO: add timestamp
            var resourceID = Strophe.getResourceFromJid(jid);
            item.resources[Strophe.getResourceFromJid(jid)] = {
                ...
                video    : resourceID ? resourceID.indexOf('vid') >= 0 : false
            };
        }
        else
        {
    ...
    _updateItem : function(item)
    {
        ...
        if (!item)
        {
            this.items.push({
                ...
                groups       : groups,
                resources    : {},
                highestResource: function(){
                    var resource = null;
                    for(var _resource in this.resources){
                        if (!resource){
                            resource = this.resources[_resource];
                        }else if (this.resources[_resource].video){
                            resource = this.resources[_resource];
                        }
                        else if(resource.priority > this.resources[_resource].priority){
                            resource = this.resources[_resource];
                        }
                    }
                    return resource;
